				...
				switch ($currentDay["monthType"])
				{
					...
					<li class="eb-calendarDay calendar-day regpro-calendarDay <?php echo $rowClass; ?>">
						...
						foreach ($currentDay["events"] as $key=> $event)
						{
							...
							if ($config->display_event_in_tooltip)
							{
								$eventLinkCssClass  = 'eb_event_link hasTooltip hasTip';
							...
							<div class="date day_cell">
								<a class="<?php echo $eventLinkCssClass; ?>" href="<?php echo JRoute::_(EventbookingHelperRoute::getEventRoute($eventId, isset($categoryId) ? $categoryId : 0, $Itemid)); ?>" title="<?php echo $eventLinkTitle; ?>"<?php if ($eventInlineStyle) echo $eventInlineStyle; ; ?>>
