			...
			<?php if ($filter_sort != false) : ?>
				<?php foreach ($filter_sort as $kfs => $fs): ?>
					<?php if (in_array($kfs, $order_display)): ?>
						...
						$j++;
						?>
						<li <?php echo $style; ?> class="fssorts" data-of="<?php echo(ltrim($fs, 'r')); ?>">
							<div class="<?php echo $subclass; ?>">
		...
		<li <?php echo $style; ?> class="last-item">
			<div class="<?php echo $subclass; ?>">
				<?php if ($params->get('auto_filter', 1) == 0): ?>
					<input class="btn btn-primary" type="submit" onclick="event.preventDefault();jQuery('#<?php echo $formid; ?>').trigger('filter.submit');" name="btnSubmit" value="<?php echo JText::_('JAK2SEARCH'); ?>"/>
				<?php endif; ?>
				<?php if ($params->get('enable_reset_button', 1) == 1): ?>
					<input class="btn btn-default" type="button" name="btnReset" value="<?php echo JText::_('RESET'); ?>" onclick="jaK2Reset('<?php echo $module->id; ?>', '<?php echo $formid; ?>', true);"/>
				<?php endif; ?>
